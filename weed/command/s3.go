// Package command 包含 S3 API 服务器的启动命令实现
// S3 API Server 提供 AWS S3 兼容的对象存储接口，负责：
// 1. 实现 AWS S3 API 协议（PutObject、GetObject、ListBuckets 等）
// 2. 将 S3 的 bucket/object 模型映射到 Filer 的目录/文件
// 3. 支持 S3 认证和权限管理（基于 IAM）
// 4. 支持 S3 的高级特性（版本控制、生命周期、CORS 等）
package command
...
var (
	// s3StandaloneOptions S3 独立服务器的配置选项
	s3StandaloneOptions S3Options
)

// S3Options S3 API 服务器的配置选项
// S3 服务器依赖 Filer 来存储实际的文件和元数据
type S3Options struct {
	filer                     *string               // Filer 服务器地址
	bindIp                    *string               // 绑定的 IP 地址
	port                      *int                  // HTTP 服务端口，默认 8333
	portHttps                 *int                  // HTTPS 服务端口
	portGrpc                  *int                  // gRPC 服务端口
	config                    *string               // 配置文件路径
	iamConfig                 *string               // IAM 配置文件路径
	domainName                *string               // 域名后缀（用于虚拟主机风格的访问）
	allowedOrigins            *string               // CORS 允许的源
	tlsPrivateKey             *string               // TLS 私钥文件路径
	tlsCertificate            *string               // TLS 证书文件路径
	tlsCACertificate          *string               // TLS CA 证书文件路径
	tlsVerifyClientCert       *bool                 // 是否验证客户端证书
	metricsHttpPort           *int                  // Prometheus 指标端口
	metricsHttpIp             *string               // Prometheus 指标 IP
	allowEmptyFolder          *bool                 // 是否允许空文件夹
	allowDeleteBucketNotEmpty *bool                 // 是否允许删除非空 bucket
	auditLogConfig            *string               // 审计日志配置文件路径
	localFilerSocket          *string               // 本地 Filer Unix Socket
	dataCenter                *string               // 数据中心名称
	localSocket               *string               // 本地 Unix Socket 路径
	certProvider              certprovider.Provider // TLS 证书提供者
	idleTimeout               *int                  // 连接空闲超时时间（秒）
}
