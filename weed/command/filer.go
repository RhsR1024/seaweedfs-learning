// Package command 包含 Filer 的启动命令实现
// Filer 是 SeaweedFS 的文件系统接口，负责：
// 1. 提供类似 POSIX 的文件系统接口（目录树、文件元数据）
// 2. 将文件切分/合并，与 Volume Server 交互
// 3. 支持多种元数据存储后端（MySQL、PostgreSQL、Redis、Cassandra 等）
// 4. 提供 WebDAV、S3、SFTP 等多种协议接口
package command
...
var (
	// f Filer 的配置选项
	f                  FilerOptions
	// filerStartS3 是否同时启动 S3 服务
	filerStartS3       *bool
	// filerS3Options S3 服务的配置选项
	filerS3Options     S3Options
	// filerStartWebDav 是否同时启动 WebDAV 服务
	filerStartWebDav   *bool
	// filerWebDavOptions WebDAV 服务的配置选项
	filerWebDavOptions WebDavOption
	// filerStartIam 是否同时启动 IAM 服务
	filerStartIam      *bool
	// filerIamOptions IAM 服务的配置选项
	filerIamOptions    IamOptions
	// filerStartSftp 是否同时启动 SFTP 服务
	filerStartSftp     *bool
	// filerSftpOptions SFTP 服务的配置选项
	filerSftpOptions   SftpOptions
)

// FilerOptions Filer 的配置选项
// Filer 可以同时启动多个协议接口（S3、WebDAV、SFTP等）
type FilerOptions struct {
	masters                 *pb.ServerDiscovery // Master Server 发现服务
	mastersString           *string            // Master Server 地址列表
	ip                      *string            // Filer 的 IP 地址
	bindIp                  *string            // 绑定的 IP 地址
	port                    *int               // HTTP 服务端口，默认 8888
	portGrpc                *int               // gRPC 服务端口
	publicPort              *int               // 只读端口（对外公开）
	filerGroup              *string            // Filer 组名（同组共享元数据）
	collection              *string            // 默认的 Collection 名称
	defaultReplicaPlacement *string            // 默认的复制策略
	disableDirListing       *bool              // 是否禁用目录列表
	maxMB                   *int               // 文件切分大小（MB）
	dirListingLimit         *int               // 目录列表数量限制
	dataCenter              *string            // 数据中心名称
	rack                    *string            // 机架名称
	enableNotification      *bool              // 是否启用通知
	disableHttp             *bool              // 是否禁用 HTTP
	cipher                  *bool              // 是否加密 Volume 数据
	metricsHttpPort         *int               // Prometheus 指标端口
	metricsHttpIp           *string            // Prometheus 指标 IP
	saveToFilerLimit        *int               // 小于此大小的文件直接保存在 Filer
	defaultLevelDbDirectory *string            // 默认 LevelDB 存储目录
	concurrentUploadLimitMB *int               // 并发上传大小限制（MB）
	debug                   *bool              // 是否启用调试模式
	debugPort               *int               // 调试端口
	localSocket             *string            // 本地 Unix Socket 路径
	showUIDirectoryDelete   *bool              // UI 是否显示目录删除按钮
	downloadMaxMBps         *int               // 下载速度限制（MB/s）
	diskType                *string            // 磁盘类型
	allowedOrigins          *string            // CORS 允许的源
	exposeDirectoryData     *bool              // 是否暴露目录数据
	certProvider            certprovider.Provider // TLS 证书提供者
}
